...
export function buildConfig(env: NodeJS.ProcessEnv) {
  ...
  return {
    ...
    modelAvailabilityProbeTpm: Math.max(1, Math.min(60, Math.trunc(parseNumber(env.MODEL_AVAILABILITY_PROBE_TPM, 1)))),
    modelAvailabilityProbeMaxPerFiveMinutes: Math.max(1, Math.min(100, Math.trunc(parseNumber(env.MODEL_AVAILABILITY_PROBE_MAX_PER_FIVE_MINUTES, 5)))),
    modelAvailabilityProbePrompts: parseCsvList(env.MODEL_AVAILABILITY_PROBE_PROMPTS),
